...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("[1][2] = " + myArray2[1][2]);

        for (int i = 0; i < myArray2.length; i++) { // 행의 갯수 바깥
            for (int j = 0; j < myArray2[i].length; j++) { // 열의 갯수
                System.out.println(myArray2[i][j]);
            }
        }

        for (int[] ints : myArray2) { // for each
            for (int anInt : ints) {
...
//          모든 원소를 1로 변경하고, 대각 원소는 10으로 변경하시오.
        ...
        for (int i = 0; i < testArray1.length; i++) {
            for (int j = 0; j < testArray1[i].length; j++) {
                testArray1[i][j] = 1; // 모든원소 1로 변경

                if (i == j) { // 대각 원소는 {0, 0) , {1, 1) ... 인덱스 번호가 같다
                    testArray1[i][j] = 10;
                }
            }
        }
